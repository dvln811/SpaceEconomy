/**
 * Planet Texture Worker
 * Receives: {id, planetType, isPlanet, seed}
 * Returns: {id, width, height, pixels} (Uint8ClampedArray transferred)
 */

// 3D Simplex Noise (same as main module)
class PlanetNoise {
  constructor(seed) {
  ...
  n3(x, y, z) {
    ...
    const gi0=this.perm[ii+this.perm[jj+this.perm[kk]]]%12,gi1=this.perm[ii+i1+this.perm[jj+j1+this.perm[kk+k1]]]%12;
    const gi2=this.perm[ii+i2+this.perm[jj+j2+this.perm[kk+k2]]]%12,gi3=this.perm[ii+1+this.perm[jj+1+this.perm[kk+1]]]%12;
  ...
  fbm(x,y,z,oct){let v=0,a=1,f=1,m=0;for(let i=0;i<oct;i++){v+=a*this.n3(x*f,y*f,z*f);m+=a;a*=0.5;f*=2;}return v/m;}
  ridged(x,y,z,oct){let v=0,a=1,f=1,m=0;for(let i=0;i<oct;i++){let n=this.n3(x*f,y*f,z*f);n=1-Math.abs(n);n*=n;v+=a*n;m+=a;a*=0.5;f*=2;}return v/m;}
}
...
function sampleRamp(ramp, t) {
  ...
  for (let i = 0; i < ramp.length - 1; i++) {
    if (t <= ramp[i+1][0]) {
      const t0=ramp[i][0],t1=ramp[i+1][0],f=(t-t0)/(t1-t0),c0=ramp[i][1],c1=ramp[i+1][1];
...
self.onmessage = function(e) {
  const { id, planetType, isPlanet } = e.data;
  const seed = Math.abs(hashStr(id));
  ...
  for (let y = 0; y < H; y++) {
    const lat = (y/H) * Math.PI, sinL = Math.sin(lat), cosL = Math.cos(lat), latN = y/H;
    for (let x = 0; x < W; x++) {
      ...
      if (isGas) {
        ...
        h = Math.sign(bv)*Math.pow(Math.abs(bv),0.5)*0.5 + 0.5 + nd.fbm(bx*pr.sS+latN*5, by*0.5, bz*pr.sS, 4)*0.15;
      } else {
        const tw = pr.wL + pr.wM + pr.wS;
        h = (na.fbm(bx*pr.sL,by*pr.sL,bz*pr.sL,6)*pr.wL + nb.fbm(bx*pr.sM,by*pr.sM,bz*pr.sM,5)*pr.wM + nc.ridged(bx*pr.sS,by*pr.sS,bz*pr.sS,4)*pr.wS) / (tw||1);
      }
      heights[y*W+x] = h;
      if (h < hMin) hMin = h;
      if (h > hMax) hMax = h;
